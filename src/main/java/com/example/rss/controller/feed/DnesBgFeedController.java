...
public class DnesBgFeedController extends AbstractController {
    ...
    public NewsResource getDnesBgToday() {
        return newsResourceAssemblers.toModel(rssReaderService.readFeed(dnesbgToday).getEntries().get(0));
    }
    ...
    public List<NewsResource> getAllNewsToday() {
        return newsResourceAssemblers.toCollectionModel(
            rssReaderService.readFeed(dnesbgToday).getEntries()
    ...
    public NewsResource getDnesBgWorld() {
        return newsResourceAssemblers.toModel(rssReaderService.readFeed(dnesbgWorld).getEntries().get(0));
    }
    ...
    @GetMapping("/dnesbg/world/all")
    public List<NewsResource> getAllNewsWorld() {
      return newsResourceAssemblers.toCollectionModel(
          rssReaderService.readFeed(dnesbgWorld).getEntries()
      ).getContent().stream().toList();
    }
}
